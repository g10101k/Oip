...
using Oip.Security.STS.Identity.IntegrationTests.Common;
using Oip.Security.STS.Identity.IntegrationTests.Mocks;
...
public class ManageControllerTests : BaseClassFixture
{
    ...
    public async Task UserIsAbleToUpdateProfile()
    {
        ...
        // Update profile
        var requestWithAntiForgeryCookie =
            RequestHelper.CreatePostRequestWithCookies(manageAction, manageProfileData, manageResponse);
        var requestWithIdentityCookie =
            CookiesHelper.CopyCookiesFromResponse(requestWithAntiForgeryCookie, registerResponse);
