    ...
    where TDbContext : DbContext, IAdminConfigurationDbContext
{
    ...
    public virtual async Task<PagedList<IdentityResource>> GetIdentityResourcesAsync(string search, int page = 1,
        int pageSize = 10)
    {
        ...
        pagedList.TotalCount = await DbContext.IdentityResources.WhereIf(!string.IsNullOrEmpty(search), searchCondition)
            .CountAsync();
    ...
    public virtual async Task<PagedList<IdentityResourceProperty>> GetIdentityResourcePropertiesAsync(
        int identityResourceId, int page = 1, int pageSize = 10)
    {
        var pagedList = new PagedList<IdentityResourceProperty>();

        var properties = await DbContext.IdentityResourceProperties
            .Where(x => x.IdentityResource.Id == identityResourceId).PageBy(x => x.Id, page, pageSize)
            .ToListAsync();

        pagedList.Data.AddRange(properties);
        pagedList.TotalCount = await DbContext.IdentityResourceProperties
            .Where(x => x.IdentityResource.Id == identityResourceId).CountAsync();
        pagedList.PageSize = pageSize;
    ...
    public virtual async Task<int> AddIdentityResourcePropertyAsync(int identityResourceId,
        IdentityResourceProperty identityResourceProperty)
    {
        var identityResource =
            await DbContext.IdentityResources.Where(x => x.Id == identityResourceId).SingleOrDefaultAsync();
        ...
        IdentityResourceProperty identityResourceProperty)
    {
        var propertyToDelete = await DbContext.IdentityResourceProperties
            .Where(x => x.Id == identityResourceProperty.Id).SingleOrDefaultAsync();
    ...
    public virtual async Task<bool> CanInsertIdentityResourcePropertyAsync(
        IdentityResourceProperty identityResourceProperty)
    {
        var existsWithSameName = await DbContext.IdentityResourceProperties.Where(x =>
            x.Key == identityResourceProperty.Key
    ...
    public virtual async Task<bool> CanInsertIdentityResourceAsync(IdentityResource identityResource)
    {
        ...
        {
            var existsWithSameName = await DbContext.IdentityResources.Where(x => x.Name == identityResource.Name)
                .SingleOrDefaultAsync();
            return existsWithSameName == null;
        }
        else
        {
            var existsWithSameName = await DbContext.IdentityResources
                .Where(x => x.Name == identityResource.Name && x.Id != identityResource.Id).SingleOrDefaultAsync();
            return existsWithSameName == null;
        }
    }
    ...
    public virtual async Task<int> DeleteIdentityResourceAsync(IdentityResource identityResource)
    {
        var identityResourceToDelete =
            await DbContext.IdentityResources.Where(x => x.Id == identityResource.Id).SingleOrDefaultAsync();
    ...
    {
        var identityClaims = await DbContext.IdentityClaims.Where(x => x.IdentityResource.Id == identityResource.Id)
            .ToListAsync();
