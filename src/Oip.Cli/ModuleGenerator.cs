        ...
        string russianI18nPath)
    {
        ...
        if (File.Exists(englishI18nPath) && !force)
        {
            throw new CliException(
                $"English translations already exist: {englishI18nPath}. Use --force to overwrite them.");
        }

        if (File.Exists(russianI18nPath) && !force)
        {
            throw new CliException(
                $"Russian translations already exist: {russianI18nPath}. Use --force to overwrite them.");
        }
        ...
        foreach (var controllerFile in Directory.GetFiles(controllersDirectory, "*.cs"))
        {
            if (File.ReadAllText(controllerFile).Contains(controllerRoute, StringComparison.Ordinal) &&
                !Path.GetFullPath(controllerFile)
                    .Equals(Path.GetFullPath(controllerPath), StringComparison.OrdinalIgnoreCase))
            {
                throw new CliException(
                    $"Controller route already exists in {controllerFile}: {module.ControllerRoute}");
            }
        }
    }
