...
public static class IdentityServerBuilderExtensions
{
        ...
        IConfiguration configuration)
    {
        var certificateConfiguration =
            configuration.GetSection(nameof(CertificateConfiguration)).Get<CertificateConfiguration>();
        var azureKeyVaultConfiguration = configuration.GetSection(nameof(AzureKeyVaultConfiguration))
        ...
        if (certificateConfiguration.UseSigningCertificateThumbprint)
        {
            ...
            var certCollection = certStore.Certificates.Find(X509FindType.FindByThumbprint,
                certificateConfiguration.SigningCertificateThumbprint, validOnly);
            if (certCollection.Count == 0) throw new Exception(CertificateNotFound);
        ...
        {
            var x509Certificate2Certs = AzureKeyVaultHelpers.GetCertificates(azureKeyVaultConfiguration).GetAwaiter()
                .GetResult();
        ...
        else if (certificateConfiguration.UseSigningCertificatePfxFile)
        {
            ...
            if (File.Exists(certificateConfiguration.SigningCertificatePfxFilePath))
                ...
                {
                    throw new Exception(
                        "There was an error adding the key file - during the creation of the signing key", e);
                }
            else
                throw new Exception(
                    $"Signing key file: {certificateConfiguration.SigningCertificatePfxFilePath} not found");
        }
        ...
        IConfiguration configuration)
    {
        ...
        if (certificateConfiguration.UseValidationCertificateThumbprint)
        {
            ...
            var certCollection = certStore.Certificates.Find(X509FindType.FindByThumbprint,
                certificateConfiguration.ValidationCertificateThumbprint, false);
            if (certCollection.Count == 0) throw new Exception(CertificateNotFound);
        ...
        else if (certificateConfiguration.UseValidationCertificatePfxFile)
        {
            ...
            if (File.Exists(certificateConfiguration.ValidationCertificatePfxFilePath))
                try
                {
                    builder.AddValidationKey(new X509Certificate2(
                ...
                {
                    throw new Exception(
                        "There was an error adding the key file - during the creation of the validation key", e);
                }
            else
                throw new Exception(
                    $"Validation key file: {certificateConfiguration.ValidationCertificatePfxFilePath} not found");
        }
