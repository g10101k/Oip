...
public class ConfigurationController : BaseController
{
    ...
    {
        if (id.IsNotPresentedValidNumber()) return NotFound();
    ...
    public async Task<IActionResult> Client(ClientDto client)
    {
        ...
        if (client.Id == 0)
        {
            ...
            SuccessNotification(string.Format(_localizer["SuccessAddClient"], client.ClientId),
        ...
        SuccessNotification(string.Format(_localizer["SuccessUpdateClient"], client.ClientId),
            _localizer["SuccessTitle"]);
    ...
    public async Task<IActionResult> ClientClone(ClientCloneDto client)
    {
        ...
        SuccessNotification(string.Format(_localizer["SuccessClientClone"], client.ClientId),
            _localizer["SuccessTitle"]);
    ...
    {
        if (!ModelState.IsValid) return View(apiScopeProperty);

        await _apiScopeService.AddApiScopePropertyAsync(apiScopeProperty);
        SuccessNotification(
            string.Format(_localizer["SuccessAddApiScopeProperty"], apiScopeProperty.Key,
                apiScopeProperty.ApiScopeName), _localizer["SuccessTitle"]);
    ...
    {
        if (!ModelState.IsValid) return View(apiResourceProperty);

        await _apiResourceService.AddApiResourcePropertyAsync(apiResourceProperty);
        SuccessNotification(
            string.Format(_localizer["SuccessAddApiResourceProperty"], apiResourceProperty.Key,
                apiResourceProperty.ApiResourceName), _localizer["SuccessTitle"]);
    ...
    {
        if (!ModelState.IsValid) return View(identityResourceProperty);

        await _identityResourceService.AddIdentityResourcePropertyAsync(identityResourceProperty);
        SuccessNotification(
            string.Format(_localizer["SuccessAddIdentityResourceProperty"], identityResourceProperty.Value,
                identityResourceProperty.IdentityResourceName), _localizer["SuccessTitle"]);

        return RedirectToAction(nameof(IdentityResourceProperties),
            new { Id = identityResourceProperty.IdentityResourceId });
    }
    ...
    {
        if (!ModelState.IsValid) return View(clientProperty);

        await _clientService.AddClientPropertyAsync(clientProperty);
        SuccessNotification(
            string.Format(_localizer["SuccessAddClientProperty"], clientProperty.ClientId, clientProperty.ClientName),
            _localizer["SuccessTitle"]);
    ...
    {
        if (!ModelState.IsValid) return View(clientClaim);

        await _clientService.AddClientClaimAsync(clientClaim);
        SuccessNotification(
            string.Format(_localizer["SuccessAddClientClaim"], clientClaim.Value, clientClaim.ClientName),
            _localizer["SuccessTitle"]);
    ...
    [HttpPost]
    [ValidateAntiForgeryToken]
    public async Task<IActionResult> IdentityResourcePropertyDelete(
    ...
    {
        if (!ModelState.IsValid) return View(identityResource);
        ...
        SuccessNotification(string.Format(_localizer["SuccessAddIdentityResource"], identityResource.Name),
            _localizer["SuccessTitle"]);
    ...
    {
        if (!ModelState.IsValid) return View(apiResource);
        ...
        ComboBoxHelpers.PopulateValuesToList(apiResource.AllowedAccessTokenSigningAlgorithmsItems,
        ...
        SuccessNotification(string.Format(_localizer["SuccessAddApiResource"], apiResource.Name),
            _localizer["SuccessTitle"]);
    ...
    {
        if (!ModelState.IsValid) return View(apiSecret);
    ...
    {
        if (!ModelState.IsValid) return View(apiScope);
