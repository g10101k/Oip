...
<body>
  <main class="oip-login-page">
    <section class="oip-login-shell" aria-label="${msg("loginTitle", (realm.displayName!realm.name))}">
      <div class="oip-login-card">
        <header class="oip-login-header">
          <svg
            aria-hidden="true"
            class="oip-login-logo"
        ...
        <#if message?has_content && (message.type != 'warning' || !isAppInitiatedAction??)>
          <div class="oip-alert oip-alert-${message.type}" role="alert">
            <span>${kcSanitize(message.summary)?no_esc}</span>
          </div>
        </#if>

        <form action="${url.loginAction}" class="oip-login-form" id="kc-form-login" method="post" onsubmit="document.getElementById('kc-login').disabled = true; return true;">
          <#if !usernameHidden??>
            <div class="oip-field">
              <label for="username">
              ...
              <#if messagesPerField.existsError('username','password')>
                <span class="oip-field-error">${kcSanitize(messagesPerField.getFirstError('username','password'))?no_esc}</span>
              </#if>
            </div>
          </#if>
          ...
          <div class="oip-field">
            ...
            <div class="oip-password-field">
              <input
                ...
                type="password">
              <button aria-label="Toggle password visibility" class="oip-password-toggle" data-password-toggle type="button">
          ...
          <div class="oip-login-options">
            <#if realm.rememberMe && !usernameEditDisabled??>
              <label class="oip-checkbox" for="rememberMe">
                <input
          ...
          <input class="oip-submit" id="kc-login" name="login" type="submit" value="${msg('doLogIn')}">
        </form>

        <#if realm.password && social?? && social.providers?has_content>
          <div class="oip-social">
            <div class="oip-divider"><span>${msg("identity-provider-login-label")}</span></div>
            <div class="oip-social-list">
              <#list social.providers as p>
                <a class="oip-social-link" href="${p.loginUrl}" id="social-${p.alias}">
  ...
  <script>
    ...
    const passwordToggleText = document.querySelector('[data-password-toggle-text]');
