  ...
  <main class="oip-login-page">
    <section class="oip-login-shell" aria-label="${msg("loginTitle", (realm.displayName!realm.name))}">
      <div class="oip-login-card">
        ...
        <div class="oip-keycloak-form">
          <#nested "form">
        </div>

        <#if auth?has_content && auth.showTryAnotherWayLink() && showAnotherWayIfPresent>
          <form action="${url.loginAction}" method="post" class="oip-try-another-way">
            <input type="hidden" name="tryAnotherWay" value="on">
            <button class="oip-link-button" type="submit">${msg("doTryAnotherWay")}</button>
          </form>
        </#if>

        <#if displayInfo>
          <div class="oip-login-info">
            <#nested "info">
          </div>
        </#if>
