...
public class AzureKeyVaultService
{
    ...
    {
        if (azureKeyVaultConfiguration == null) throw new ArgumentException("missing azureKeyVaultConfiguration");
    ...
    public async Task<(X509Certificate2 ActiveCertificate, X509Certificate2 SecondaryCertificate)>
        GetCertificatesFromKeyVault()
    {
        ...
        if (certificateItems.Count > 1)
            certs.SecondaryCertificate =
                await GetCertificateAsync(certificateItems[1].Identifier.Identifier, keyVaultClient);
    ...
    public IKeyVaultClient BuildKeyVaultClient()
    {
        ...
        if (_azureKeyVaultConfiguration.UseClientCredentials)
        {
            ...
            {
                var adCredential = new ClientCredential(_azureKeyVaultConfiguration.ClientId,
    ...
    private async Task<List<CertificateItem>> GetAllEnabledCertificateVersionsAsync(IKeyVaultClient keyVaultClient)
    {
        ...
        var certificateVersions = await keyVaultClient.GetCertificateVersionsAsync(
            _azureKeyVaultConfiguration.AzureKeyVaultEndpoint,
